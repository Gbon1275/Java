	...
	public TextMenu(DrawPanel owner) {
		...
		lineSpacing.addActionListener( new ActionListener() {
			public void actionPerformed(ActionEvent evt) {
				...
				String str = JOptionPane.showInputDialog(panel, "Set spacing",currentSpacing);
				if (str != null && str.trim().length() > 0) {
					try {
						double newSpacing = Double.parseDouble(str.trim()); // can throw NumberFormatException
						panel.getTextItem().setLineHeightMultiplier(newSpacing); // can throw IllegalArgumentException
						panel.repaint();
					}
					catch (Exception e) {
						JOptionPane.showMessageDialog(panel, str + " is not a legal line height value.\n"
		...
		final JMenu justify = JustifyMenu();
		add(justify);
		addSeparator();
		add(makeFontNameSubmenu());
	}
